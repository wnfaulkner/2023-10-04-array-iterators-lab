...
// 1. Filter the array of inventors into a new array containing only the inventors born in the 1500's
    const sixteenthCenturyInventors = inventors.filter((inventor) => inventor.year >=1500 && inventor.year < 1600)
    // console.log(sixteenthCenturyInventors)
...
// Hint:  Return a new object literal from the callback (don't mutate the object being passed in to map)
    ...
    const inventorNames = inventors.map(
        function(inventor){ //HOW TO CHANGE TO ARROW FUNCTION?
...
// 3. Sort the inventors by birth date (year property), in ascending order
    const inventorsSortedByBirthday = inventors.sort(
        (inventor1, inventor2) => inventor1.year - inventor2.year //WHAT'S UP WITH SUGGESTIONS TO USE PARSEFLOAT WHEN THIS SEEMS TO WORK JUST FINE?
    )
...
// 5. How many years did all the inventors live?
    ...
    const anyDevsOlderThan19 = devs.some(
        function(dev) {
            const currentYear = new Date().getFullYear() 
            dev.age = currentYear - dev.year
            dev.olderThan19 = dev.age > 19
            return(dev.olderThan19)
        }
    )
    // console.log(devs)
    // console.log(anyDevsOlderThan19)

    // Array.prototype.every()
    // 9. Check if everyone is 19 or older?
    
    const everyoneOlderThanNineteen = devs.every(
